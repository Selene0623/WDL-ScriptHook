...
int* BlankMemory = nullptr;
void* MDLE = GetModuleHandleA("DuniaDemo_clang_64_dx12.dll");

BYTE* RVAtoVA(void* base, UINT_PTR offset) {
    BYTE* a1 = reinterpret_cast<BYTE*>(base) + offset;
    return a1;
}
...
void* getOffset(unsigned int a2) {
    switch (a2) {
        case 0x1065a744: {
            LOG(" Hash Redirected.");
        ...
        case 0x113676f0: {
            LOG("Unsupported Module, hash 0x113676f0\n");
        ...
        case 0xa93eead8: {
            LOG("Warning. Attempt to patch -battleye section when it doesn't exist anymore. Redirected to null void. \n");
            return &BlankMemory;
            break;
        }
